  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const message = err.response?.data?.message || "Failed to fetch classes or users";
...
const handleJoin = async () => {
  ...
  try {
    ...
    // ✅ Re-map teacher info (IMPORTANT)
    const classesWithTeachers = refreshedClasses.map(c => {
      const teacher = users.find(
        u => u.id === c.teacherId && u.role === 'teacher'
      );
    ...
    <div className="space-y-8">
          ...
          {user?.role === "teacher" && (
            ...
            >
              ...
              <span className="text-xs">
                {user.isVerified ? "INITIALIZE CLASS" : "AWAITING VERIFICATION"}
              </span>
            </button>
          )}
          {user?.role === "student" && (
            ...
            >
              <Code size={18} />
              <span className="text-xs">
                {user.isVerified ? "JOIN CLASS" : "AWAITING VERIFICATION"}
              </span>
            </button>
          )}
        </div>
      ...
      {!user?.isVerified && user?.role !== "admin" && (
        <div className="glass-panel p-4 lg:p-6 border-yellow-500/30 bg-yellow-500/5 flex items-center gap-4">
          ...
          <div>
            <p className="text-yellow-500 font-bold uppercase tracking-widest text-[10px] sm:text-sm">
              Verification Required
            </p>
            <p className="text-[10px] sm:text-xs text-white/60">
              Your neural signature is pending administrator authorization.
              Access to academic sectors is restricted.
            </p>
          </div>
        </div>
      )}
                ...
                >
                  ...
                  <div className="p-6 flex-1 flex flex-col">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-xl font-bold tracking-tight group-hover:text-neon-blue transition-colors">
                          {cls.name}
                        </h3>
                        <p className="text-[10px] font-mono text-neon-blue/60 mt-1">
                          CODE: {cls.inviteCode || "N/A"}
                        </p>
                      </div>
                    ...
                    {cls.stats && (
                      <div className="grid grid-cols-3 gap-2 mb-6">
                        ...
                        <div className="bg-emerald-500/5 rounded-lg p-2 text-center border border-emerald-500/10">
                          <p className="text-[10px] text-emerald-500/40 uppercase mb-1">
                            Done
                          </p>
                        ...
                        <div className="bg-orange-500/5 rounded-lg p-2 text-center border border-orange-500/10">
                          <p className="text-[10px] text-orange-500/40 uppercase mb-1">
                            Pending
                          </p>
                      ...
                      {user?.role === "student" && !cls.isEnrolled ? (
                        ...
                        <button
                          onClick={() => {
                            const id = cls?.id;
                            if (!id) {
                              const message = "Failed to navigate to class: Invalid class ID";
                              const { useUIStore } = import("../store.js");
                              useUIStore.getState().showNotification(message, "error", 8000);
