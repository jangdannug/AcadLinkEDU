...
export default function ClassDetail() {
  ...
  const [selectedActivityForUpload, setSelectedActivityForUpload] =
    useState(null);
  const [uploadFiles, setUploadFiles] = useState({});
  const [trackingData, setTrackingData] = useState([]);
  const [showTrackingModal, setShowTrackingModal] = useState(false);
  const [selectedActivityForTracking, setSelectedActivityForTracking] =
    useState(null);
  ...
  useEffect(() => {
    const fetchData = async () => {
      const [classes, allActivities, allUsers, allSubmissions, analyticsRes] =
        await Promise.all([
          api.getClasses(),
          api.getActivities(),
          api.getUsers(),
      ...
      const classActivities = allActivities.filter(
        (a) => a.classId === Number(classId),
      );
  ...
  const handleCreateActivity = async () => {
    let data;
    if (editingActivity) {
      data = await api.updateActivity(editingActivity.id, {
        ...newActivity,
        classId: Number(classId), // <- convert to integer
      });
  ...
  files: uploadFiles, // the object from <input type="file">
    ...
    //   studentId: user.id,
    //   fileUrl: "#",
    //   fileName: Object.values(uploadFiles)
    //     .map((f) => f?.name)
    //     .filter(Boolean)
    //     .join(", "),
    // });
    ...
    alert(
      "Neural uplink successful. Data packets transmitted to academic sector.",
    );
  ...
  if (!classData)
    return (
      <div className="flex items-center justify-center h-full text-neon-blue animate-pulse">
        SYNCING SECTOR DATA...
      </div>
    );
    ...
    return (
      <div className="flex flex-col items-center justify-center h-full space-y-4">
        ...
        <p className="text-white/50 text-center max-w-md">
          Your neural signature is pending administrator authorization. Access
          to academic sectors is restricted until verification is complete.
        </p>
        <button onClick={() => navigate("/classes")} className="cyber-button">
          RETURN TO MATRIX
        </button>
      </div>
    );
        ...
        <div className="flex justify-between items-end">
          <div>
            <h2 className="text-[10px] uppercase tracking-[0.3em] text-neon-blue font-bold mb-2">
              Class Sector: {classData.inviteCode}
            </h2>
          ...
          >
            ...
            {activities.map((activity, i) => {
              const sub = submissions.find(
                (s) => s.activityId === activity.id && s.studentId === user.id,
              );
              ...
              return (
                <div
                  key={activity.id}
                  className="glass-panel p-6 flex items-center justify-between border-l-4 border-l-neon-blue"
                >
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-6 text-[10px] text-white/30 uppercase tracking-widest">
                      <div className="flex items-center gap-2">
                        ...
                        <span>
                          Deadline:{" "}
                          {format(parseISO(activity.deadline), "MMM dd, HH:mm")}
                        </span>
                      </div>
                      <div className="flex items-center gap-2">
                        <FileText size={12} />
                        <span>
                          Required:{" "}
                          {activity.requiredFiles?.join(", ").toUpperCase() ||
                            "NONE"}
                        </span>
                      </div>
                    </div>
                  ...
                  <div className="flex gap-3">
                    {user?.role === "teacher" ? (
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => {
                            setSelectedActivityForTracking(activity);
                    ...
                    <div className="mt-3">
                      ...
                      <div className="h-1 bg-white/5 rounded-full overflow-hidden">
                        <motion.div
                          initial={{ width: 0 }}
                          animate={{
                            width: `${getStudentProgress(student.id)}%`,
                          }}
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold neon-text">
                Mission Tracking: {selectedActivityForTracking.title}
              </h2>
              <button
            ...
            <div className="space-y-4">
              {trackingData.map((data) => {
                const status = data.activities.find(
                  (a) => a.activityId === selectedActivityForTracking.id,
                );

                const submission = submissions.find(
                  (s) =>
                    s.activityId === selectedActivityForTracking.id &&
                        ...
                        <div className="flex flex-col items-end">
                          ...
                          {submission?.fileUrl && (
                            <button
                              ...
                              className="text-[10px] text-neon-blue underline flex items-center gap-1"
                            >
                              <FileText size={12} />
                              Preview
                            </button>
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setNewActivity({ ...newActivity, deadline: e.target.value })
                  }
        ...
        {selectedActivityForUpload && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
            ...
            >
              ...
              <div className="flex justify-between items-start mb-8">
                <div>
                  <h2 className="text-3xl font-bold tracking-tighter mb-2">
                    {selectedActivityForUpload.title}
                  </h2>
                  <div className="flex items-center gap-4 text-[10px] text-white/40 uppercase tracking-[0.2em]">
                    <span className="flex items-center gap-1">
                      ...
                      Deadline:{" "}
                      {format(
                        parseISO(selectedActivityForUpload.deadline),
                        "MMM dd, yyyy HH:mm",
                      )}
                    </span>
                  </div>
              ...
              <div className="space-y-8">
                <div>
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-neon-blue mb-3">
                    Mission Parameters
                  </h4>
                  <p className="text-white/70 leading-relaxed">
                    {selectedActivityForUpload.description}
                  </p>
                </div>

                <div>
                  <h4 className="text-[10px] font-bold uppercase tracking-widest text-neon-blue mb-4">
                    Required Data Uplink
                  </h4>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {selectedActivityForUpload.requiredFiles?.map((type) => (
                      <div
                        key={type}
                        className="bg-white/5 border border-white/10 rounded-xl p-4 flex items-center justify-between group hover:border-neon-blue/30 transition-all"
                      >
                  ...
                  {Object.keys(uploadFiles).length > 0 && (
                    <div className="mt-4 p-4 bg-neon-blue/5 border border-neon-blue/20 rounded-xl">
                      <h5 className="text-[10px] font-bold text-neon-blue uppercase mb-2">
                        Staged for Uplink:
                      </h5>
                      <div className="space-y-1">
                        {Object.entries(uploadFiles).map(
                          ([type, file]) =>
                            file && (
                              <div
                                key={type}
                                className="text-[10px] text-white/60 flex justify-between"
                              >
                                <span>
                ...
                <div className="pt-6 border-t border-white/5 flex gap-4">
                  ...
                  <button
                    onClick={() =>
                      handleFileUpload(selectedActivityForUpload.id)
                    }
